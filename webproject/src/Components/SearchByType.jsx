...
export default class SearchByType extends Component {
    ...
    getData = async (pageNumber) => {
        ...
        console.log("test2", window.location.href.slice(35));
        let type = window.location.href.slice(35);
        try {
            const result = await fetch(
                `http://localhost:5000/products/getItemsByType?pageNumber=${pageNumber}&pageSize=${pageSize}&type=${type}`,
                {
                    method: 'GET',
                    headers: {
