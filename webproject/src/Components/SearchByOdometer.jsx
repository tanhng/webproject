...
const pageSize = 8;
export default class searchByOdometer extends Component {
    constructor(props) {
        super(props)
    ...
    getData = async (pageNumber) => {
        ...
        console.log("test2", window.location.href.slice(39));
        let odometer = window.location.href.slice(39);

        try {
            const result = await fetch(
                `http://localhost:5000/products/getItemsByOdometer?pageNumber=${pageNumber}&pageSize=${pageSize}&odometer=${odometer}`,
                {
                    method: 'GET',
                    headers: {
